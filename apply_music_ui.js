...
const playReplace =     // Playback Logic
    ...
    (this as any).updateSeekUIRef = (seek: number, duration: number) => {
      ...
      // User wanted time left along with time beside it. 
      // Time left = duration - seek. So we show "Seek / -Left"
    ...
    const changeTrack = (dir: number) => {
      ...
      let newMusic = this.sound.get(curKey) as Phaser.Sound.WebAudioSound;
      if (!newMusic) newMusic = this.sound.add(curKey) as Phaser.Sound.WebAudioSound;
    ...
    const applySeek = () => {
      ...
      const music = this.sound.get(curKey) as Phaser.Sound.WebAudioSound;
      if (music && music.isPlaying) {
        const vol = parseFloat(volumeSlider.value);
